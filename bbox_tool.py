...
class LabelTool:
    ...
    def loadImage(self):
        ...
        # Calculate the new image dimensions based on basewidth
        wpercent = (basewidth / float(self.img.size[0]))
        hsize = int((float(self.img.size[1]) * float(wpercent)))
        
        # Resize the image using LANCZOS filter
        img = self.img.resize((basewidth, hsize), Image.Resampling.LANCZOS)
        
        # Convert the image for use in Tkinter
        self.tkimg = ImageTk.PhotoImage(img)
        
        # Update the mainPanel with the new image
        ...
        if os.path.exists(self.labelfilename):
            with open(self.labelfilename) as f:
                for (i, line) in enumerate(f):
                    ...
                    # Handle non-YOLO format bounding boxes
                    if line[1].isdecimal():
                        self.save_to_yolo_format.set(0)
                        tmp = line.split()
                        self.bboxList.append(tuple(tmp))
                        tmp_id = self.mainPanel.create_rectangle(
                    ...
                    # Handle YOLO format bounding boxes
                    else:
                        ...
                        tmp_id = self.mainPanel.create_rectangle(
                            tmp_alt[0], tmp_alt[1], tmp_alt[2], tmp_alt[3],
                            width=2, outline=COLORS[(len(self.bboxList) - 1) % len(COLORS)])
                        self.bboxIdList.append(tmp_id)
                        self.listbox.insert(END, '%s: %.3f, %.3f , %.3f, %.3f' % (cls, float(tmp[1]), float(tmp[2]), float(tmp[3]), float(tmp[4])))
                        self.listbox.itemconfig(len(self.bboxIdList) - 1, fg=COLORS[(len(self.bboxIdList) - 1) % len(COLORS)])
